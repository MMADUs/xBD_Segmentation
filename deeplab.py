...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        image_feat = F.interpolate(image_feat, size=(h, w), mode='bilinear', align_corners=True)
...
class DeepLabDecoder(nn.Module):
    def __init__(self, low_channels, out_channels=48, final_channels=256):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(out_channels + final_channels, final_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(self, low_feat, high_feat):
        ...
        high = F.interpolate(high_feat, size=low.size()[2:], mode='bilinear', align_corners=False)
...
class DeepLabV3Plus(nn.Module):
    def __init__(self, num_classes=4, pretrained=True):
        ...
        self.decoder = DeepLabDecoder(low_channels=256, out_channels=48, final_channels=256)
